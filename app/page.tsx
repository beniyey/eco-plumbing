...
function FadeInSection({ children, className = "" }: { children?: any, className?: string }) {
  ...
  // When threshold 0.3 of the component is visible, isInView becomes true.
...
export default function Home() {
  return (
    <div className="m-auto relative">
      {/* Hero Section */}
      <FadeInSection className="container flex flex-col gap-14 lg:flex-row justify-between p-6 px-10 pb-12 lg:pb-0">
        ...
        <div className="relative w-fit m-auto">
          ...
          <span className="absolute border-4 border-secondary-text w-40 h-40 block rounded-full top-0 lg:top-10 right-0 -z-10 lg:duration-300" />
          <span className="absolute border-4 border-secondary-text w-20 h-20 block rounded-full top-56 lg:top-64 left-0 -z-10 lg:duration1" />
          <span className="absolute border-4 border-white w-40 h-40 block rounded-full top-56 md:top-96 right-0 -z-50 lg:duration1" />
        </div>
        {/* Hero Content */}
        <div className="flex flex-col w-full lg:w-1/2 gap-8 text-center lg:text-right">
          ...
          <span className="flex flex-col-reverse md:flex-row justify-center lg:justify-end gap-8 items-center">
            <Link className={underlineButton} href="/contact">
              צור קשר
            </Link>
            <Link className={filledButton} href="/about">
              קצת עלינו
            </Link>
          </span>
      ...
      {/* About Us Section */}
      <FadeInSection className="w-full bg-white flex flex-col gap-24 py-14 relative">
        ...
        <div className="container flex flex-col lg:flex-row relative">
          ...
          <div className="flex flex-col text-right p-6 lg:p-20 items-center lg:items-end lg:w-1/2 gap-6">
            ...
            <h2 className="text-black text-4xl font-bold">עמידה בזמנים ומקצועיות מעל הכל</h2>
            <p>
              אנחנו, צוות מומחים באינסטלציה עם ניסיון של שנים וידע מעמיק בטיפול בכל בעיות הצנרת. אנו מתמחים באיתור נזילות, פתיחת סתימות והתקנות מתקדמות, תוך שימוש בטכנולוגיות החדשניות ביותר. החזון שלנו הוא להעניק שירות אמין, מהיר ואיכותי לכל לקוח, עם דגש על מקצועיות ושביעות רצון. כשאתם בוחרים בנו, אתם בוחרים בשקט נפשי.
            </p>
      ...
      <FadeInSection className="flex flex-col gap-20">
        <span className="z-10 w-fit m-auto flex flex-col items-center justify-center">
          ...
          <h2 className="text-black text-center text-4xl font-bold mt-4">
        ...
        <div className="flex flex-row flex-wrap gap-y-48 justify-center gap-12 mb-24">
          {/* Service Card 1 */}
          <FadeInSection className="relative min-w-80 p-4">
            <Image
              src="/images/pipe.jpg"
            ...
            />
            <span className="absolute bg-background-primary-light p-10 rounded-lg w-5/6 -bottom-24 shadow-xl left-1/2 -translate-x-1/2 flex flex-col justify-center items-end gap-1">
          ...
          <FadeInSection className="relative min-w-80 p-4">
            ...
            <span className="absolute bg-background-primary-light p-10 rounded-lg w-5/6 -bottom-24 shadow-xl left-1/2 -translate-x-1/2 flex flex-col justify-center items-end gap-1">
              ...
              <p className="text-right my-5">
                תיקון והתקנת צנרת, ברזים וכלים סניטריים עם שירות מקצועי ואמין
              </p>
              <Link className={underlineButton} href="/services/plumbing">
                לעמוד השירות
              </Link>
            </span>
          </FadeInSection>
          ...
          <FadeInSection className="relative min-w-80 p-4">
            ...
            <span className="absolute bg-background-primary-light p-10 rounded-lg w-5/6 -bottom-24 shadow-xl left-1/2 -translate-x-1/2 flex flex-col justify-center items-end gap-1">
              ...
              <p className="text-right my-5">
                פתיחת סתימות מקצועית ללא נזק, עם ציוד מתקדם ופתרון מהיר
              </p>
              <Link className={underlineButton} href="/services/drain-cleaning">
                לעמוד השירות
              </Link>
            </span>
          </FadeInSection>
        </div>
        <Link className={filledButton + " m-auto py-5 px-12"} href="/services">
          לכל השירותים
        </Link>
      </FadeInSection>

      {/* Free Estimate Section */}
      <FadeInSection className="py-32 lg:py-40 relative px-4 overflow-hidden">
      ...
      {/* Blog Section */}
      <FadeInSection className="flex flex-col gap-20 bg-white py-40">
